...
productsDirectives.directive('product', function($state, Products) {
  return {
    ...
    link: function (scope, element) {
      ...
      function init() {
        scope.currentProductSubsection;
        scope.navbarProductActive = false;

        scope.$on('activeSubsection', function(event, subsection){ scope.$apply(function() { scope.currentProductSubsection = subsection; }); })
        scope.$on('inactiveSubsection', function(event, subsection){ scope.$apply(function() { scope.currentProductSubsection = subsection; }); })
        scope.$on('activeSection:product', function(event){ scope.$apply(function() { scope.navbarProductActive = true; }); })
        scope.$on('inactiveSection:product', function(event){ scope.$apply(function() { scope.navbarProductActive = false; }); })
      }
    }  
  };
});
...
productsDirectives.directive('productNavbar', function() {
  return {
    ...
    scope: {
      product: '=', // Using the same name as the attribute
      isActive: '=',
      currentProductSubsection: '='
    },
    link: function (scope, element, attrs) {
      ...
      function init() {
        ...
        scope.$watch('isActive', function(value){
            if (value) { element.find('#product-navbar').addClass('on-navbar-active'); }
            else { element.find('#product-navbar').removeClass('on-navbar-active'); element.find('#' + currentActiveSubsection + '-link').removeClass('product-navbar-link-active'); }
        });
        scope.$watch('currentProductSubsection', function(value){
          element.find('#' + currentActiveSubsection + '-link').removeClass('product-navbar-link-active');
          currentActiveSubsection = value;
          element.find('#' + currentActiveSubsection + '-link').addClass('product-navbar-link-active');
        });
      }
    }  
  };
